...
bool InitData()
{
  ...
  else 
  {
    ...
    font_menu = TTF_OpenFont("assets//fonts//CubaoWide.ttf", 50);
...
bool LoadBackground()
{
  bool ret = g_background1.LoadImg("assets//images//background.png", g_screen);
  bool ret2 = g_background2.LoadImg("assets//images//bkg2.png", g_screen);
  if (ret == false || ret2 == false) return false;
  return true;
}
...
std::vector<ThreatObject*> MakeThreatList()
{
  ...
  ThreatObject* dynamic_threats = new ThreatObject[10];
  for (int i = 0; i < 5; ++i)
  {
  ...
  for (int i = 0; i < 20; ++i)
  {
    ...
    if (p_threat != NULL)
    {
      p_threat->LoadImg("assets//images//threat_level.png", g_screen);
      p_threat->set_clips();
...
again_label:
  ...
  MainObject p_player;
  p_player.LoadImg("assets//images//player_right.png", g_screen);
  p_player.set_clips();

  AliveIndex alive_index;
  alive_index.Init(g_screen);
  ...
  while (!is_quit)
  {
    ...
    if(mark_value >= 2000 && mark_value < 3000)
    {
      ...
      if (p_threat != NULL)
      {
        ...
        if (crash)
        {
            ...
            if (num_die < 3) 
            {
              ...
              p_player.SetBackTime(30);
              SDL_Delay(800);
              alive_index.Decrease();
              alive_index.Render(g_screen);
              continue;
            }
            else
              game_over = true;
